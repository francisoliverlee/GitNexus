      ...
      <div className="group relative">
        <button
          ...
          className={`
            ...
            ${isFocused ? 'bg-accent/10 border-l-2 !border-accent' : ''}
        ...
        >
          ...
          {node.type === 'folder' ? (
            isExpanded ? (
              <FolderOpen className="w-4 h-4 shrink-0" style={{ color: isFocused ? '#8b5cf6' : NODE_COLORS.Folder }} />
            ) : (
              <Folder className="w-4 h-4 shrink-0" style={{ color: isFocused ? '#8b5cf6' : NODE_COLORS.Folder }} />
            )
          ) : (
          ...
          {/* Focused indicator */}
          {isFocused && (
            <Eye className="w-3 h-3 shrink-0 text-accent ml-auto" />
          )}
        </button>

        {/* Focus folder button on hover (only for folders) */}
        {node.type === 'folder' && !isFocused && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              onFocusFolder(node.path);
            }}
            className="absolute right-1 top-1/2 -translate-y-1/2 p-0.5 rounded opacity-0 group-hover:opacity-100 text-text-muted hover:text-accent hover:bg-accent/10 transition-all"
            title="Focus graph on this folder"
          >
...
export const FileTreePanel = ({ onFocusNode }: FileTreePanelProps) => {
  const { graph, visibleLabels, toggleLabelVisibility, visibleEdgeTypes, toggleEdgeVisibility, selectedNode, setSelectedNode, openCodePanel, depthFilter, setDepthFilter, focusedFolderPath, setFocusedFolderPath } = useAppState();
  ...
  const handleNodeClick = useCallback((treeNode: TreeNode) => {
    ...
    // When clicking a folder, automatically scope the graph to show only this folder's contents
    if (treeNode.type === 'folder') {
      // Toggle: if clicking the already-focused folder, unfocus
      if (focusedFolderPath === treeNode.path) {
        setFocusedFolderPath(null);
      } else {
        setFocusedFolderPath(treeNode.path);
      }
    }
  }, [setSelectedNode, openCodePanel, onFocusNode, selectedNode, focusedFolderPath, setFocusedFolderPath]);

  const handleFocusFolder = useCallback((path: string) => {
    // Toggle: if clicking the same folder, unfocus
    if (focusedFolderPath === path) {
      setFocusedFolderPath(null);
    } else {
      setFocusedFolderPath(path);
    }
  }, [focusedFolderPath, setFocusedFolderPath]);

  // Navigate up one level in focused folder path
  const handleNavigateUp = useCallback(() => {
    if (!focusedFolderPath) return;
    const parentPath = focusedFolderPath.substring(0, focusedFolderPath.lastIndexOf('/'));
    if (parentPath) {
  ...
  // Build breadcrumb parts from focusedFolderPath
  const breadcrumbParts = useMemo(() => {
    if (!focusedFolderPath) return [];
    return focusedFolderPath.split('/').filter(Boolean);
  }, [focusedFolderPath]);
    ...
    <div className="h-full w-64 bg-surface border-r border-border-subtle flex flex-col animate-slide-in">
      ...
      {activeTab === 'files' && (
        <>
          ...
          {/* Focused folder breadcrumb / scope indicator */}
          {focusedFolderPath !== null && (
            <div className="px-2 py-1.5 border-b border-border-subtle bg-accent/5 flex items-center gap-1">
              <button
                onClick={handleNavigateUp}
                className="p-0.5 rounded text-text-muted hover:text-accent hover:bg-accent/10 transition-colors shrink-0"
