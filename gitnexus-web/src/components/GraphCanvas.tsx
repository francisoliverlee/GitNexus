...
import { knowledgeGraphToGraphology, filterGraphByDepth, filterGraphByFolder, SigmaNodeAttributes, SigmaEdgeAttributes } from '../lib/graph-adapter';
...
export const GraphCanvas = forwardRef<GraphCanvasHandle>((_, ref) => {
  ...
  useEffect(() => {
    ...
    // Apply folder scope filter if set, otherwise use depth filter
    if (focusedFolderPath !== null) {
      filterGraphByFolder(sigmaGraph, graph, focusedFolderPath, visibleLabels);
    } else {
  ...
  }, [visibleLabels, depthFilter, appSelectedNode, focusedFolderPath, graph]);
